...
public class TfLArrivalsParser extends TfLAbstractParser {
    ...
    public static void parseArrivals(Station stn, String jsonResponse)
        ...
        for (int i = 0; i < jsonArray.length(); i ++) {
            ...
            if (!jsonArray.getJSONObject(i).has("timeToStation")) {
            ...
            Integer timeToStation = jsonArray.getJSONObject(i).getInt("timeToStation");

            if (!jsonArray.getJSONObject(i).has("platformName")) {
            ...
            String platformName = jsonArray.getJSONObject(i).getString("platformName");

            if(!jsonArray.getJSONObject(i).has("destinationName") && !jsonArray.getJSONObject(i).has("towards")) {
            ...
            if (jsonArray.getJSONObject(i).has("destinationName")) {
                destination = TfLAbstractParser.parseName(jsonArray.getJSONObject(i).getString("destinationName"));
            }
            else {
                destination = jsonArray.getJSONObject(i).getString("towards");
            }

            Arrival arrival = new Arrival(timeToStation, destination, platformName);

            if (!jsonArray.getJSONObject(i).has("lineId")) {
            ...
            String lineId = jsonArray.getJSONObject(i).getString("lineId");
